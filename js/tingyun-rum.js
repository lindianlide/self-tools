...
!(function (win, doc) {
    ...
    var performance = win.performance ? win.performance : win.Performance; // performance
    var _ua = navigator.userAgent; // UA
    var _fuckIe = (new Function("return!+[1,]"))(); // ie8-
    var SELF_DATA = '_SELF_DATA'; // localstorege 存储 获取页面基本信息
    var SESSION_ID = "SELF_SESSION_ID"; // session id 存储在cookie 里面的 标识会话的信息。
    var DISTINCT_ID = "SELF_DISTINCT_ID"; // distinct id 存储在 localstorage 里面的标识人的信息。
    var CITY_INFO = 'SELF_CITY_INFO';// city info 存储在 localstorage 中的标识城市信息。
    ...
    var _performanceUrl = '/dc-web/performance.gif'; // 页面性能
    var _resourcePerformanceUrl = '/dc-web/resource.gif'; // 页面资源
    var _xhrUrl = '/dc-web/xhr.gif'; // ajax
    var _errorUrl = '/dc-web/error.gif'; // error
    var _appErrorUrl = '/dc-web/excp.gif'; // js业务异常上班。
    var _ipServiceUrl = 'ipservice.com/ipQuery.do'; // 获取ip信息

    // 配置各种环境的地址。
    _defaultHost = function () {
        var _hostName = doc.location.hostname;
        // 一般生产环境的域名
        var _prd_reg = /(\W)*.com$/;
        // 一般pre环境的域名
        var _pre_reg = /(\W)*pre(.*)*.cn.com$/;
        // 一般sit环境的域名
        var result = '';
        // Pre
        if (_pre_reg.test(_hostName)) {
            result = 'dc.bpmpre.cloudytrace.com';
        }
        // prd
    ...
    var isSupportLocalStorage = function () {
        ...
        try {
            win.localStorage && win.localStorage.setItem(_pvid, _pvid);
            win.localStorage && win.localStorage.removeItem(_pvid);
            result = true;
        }
        catch (e) {
            result = false;
    ...
    function _cookie(key, value, options) {
        if (!isUndefined(value)) {
            ...
            doc.cookie = [key, '=', encode(value), expires, path, domain, secure].join("");
        }
        else {
    ...
    function post(url, data, headers, callback) {
        ...
        if (win.navigator && win.navigator.sendBeacon && /^http/i.test(url)) {
            // https://developer.mozilla.org/zh-CN/docs/Web/API/Navigator/sendBeacon
            var response = win.navigator.sendBeacon(url, data);
            callback(!response);
        }
        else {
            var request;
    ...
    function wrapTimeout(name) {
        each(['setTimeout', 'setInterval'], function (item) {
            //
            wrap(true, win, item, function (it) {
                return function () {
                    var temp;
    ...
    var uuid = function () {
        ...
        function e(t, e) {
            for (var r = t.toString(16), n = e - r.length, a = "0"; n > 0; n >>>= 1,
                a += a)
                1 & n && (r = a + r);
            return r
        }
    ...
    function makeUrl(url, path) {
        var protocol = /^https/i.test(doc.URL) ? 'https' : 'http';
    ...
    function getResourceLength(obj) {
        ...
        else if (win.ArrayBuffer && obj instanceof ArrayBuffer) {
            result = obj.byteLength;
        }
        else if (win.Blob && obj instanceof Blob) {
            result = obj.size;
        }
    ...
    function startsWith(str1, prefix) {
        ...
        var func = _startsWith ? function (s, p) {
    ...
    function formatError(errors) {
        ...
        if (argsLength > 0) {
            ...
            // Error 错误类型
            else if (errorMsg instanceof Error || (win.ErrorEvent && (errorMsg instanceof win.ErrorEvent))) {
                //
                if (errorMsg.message) {
                ...
                if (errorMsg.error) {
                    result.message = (errorMsg.error.constructor.name) + (errorMsg.error.message || '');
    ...
    function formatSysPerformanceInfo() {
        ...
        if (performance && performance.timing) {
            ...
            var domainLookupStartTime = _timeDifference(PERFORMANCE_TIMING.domainLookupStart);
            var domainLookupEndTime = _timeDifference(PERFORMANCE_TIMING.domainLookupEnd);
            ...
            result = {
                ...
                domContentLoadedEventStartTime: _timeDifference(PERFORMANCE_TIMING.domContentLoadedEventStart),
                domContentLoadedEventEndTime: _timeDifference(PERFORMANCE_TIMING.domContentLoadedEventEnd),
                ...
                unloadEventStartTime: _timeDifference(PERFORMANCE_TIMING.unloadEventStart),
            ...
            if (timing.msFirstPaint) {
                ...
                var loadTime = win.chrome.loadTimes();
            ...
            if (timing.secureConnectionStart) {
                result.secureConnectionStartTime = _timeDifference(PERFORMANCE_TIMING.secureConnectionStart);
            }
        }
        else {
            result = {
    ...
    function formatResourcePerformanceInfo(loadEventStart) {
        ...
        if (performance) {
            ...
            // 如果资源存在的话
            if (resource) {
                _performanceResourceTimingBufferList = _performanceResourceTimingBufferList.concat(resource);
            }
            ...
            //
            for (var i = 0, len = _performanceResourceTimingBufferList.length; i < len; i++) {
                ...
                // 是否慢资源
                var isSlowResource = (resourceObj.responseEndTime - resourceObj.startTime) > defaultConfig.slowResourceT;
                ...
                if (isSlowResource) {
                    ...
                    if ((loadEventStart > defaultConfig.slowPageT) && (getRandom() <= defaultConfig.slowP)) {
                        result.push(resourceObj);
                    }
                }
    ...
    function getCityData(callback) {
        ...
        if (!isObjectEmpty(_cityData)) {
            callback(null);
            return;
        }

        // 先从缓存里面去获取数据
        var cityInfo = getStorage(CITY_INFO);

        if (cityInfo) {
            cityInfo = parse(cityInfo);
            var _now = now();
            var preTime = cityInfo.t;

            // 如果过期掉了
            if (_now - preTime > 5 * 24 * 60 * 1000) {
                _fetchData();
            }
            else {
                // 更新 数据
        ...
        function _fetchData() {
            var callbackName = 'snpfjsonp1';
            var abortTimeout = null;
            var script = doc.createElement('script');
            var head = doc.getElementsByTagName('head')[0] || doc.documentElement;

            addEventListener(script, 'error', function () {
                win.clearTimeout(abortTimeout);
                callback('error');
                _removeScript();
            });

            script.src = makeUrl(_ipServiceUrl) + '?callback=' + callbackName + '&_=' + now();
            script.async = true;
            ...
            win[callbackName] = function (data) {
                ...
                // 存储数据
                var storeData = {
                    p: encode(data.provinceName),
                    c: encode(data.cityName),
                    d: encode(data.districtName),
                    t: now()
                };
                _cityData = storeData;
                setStorage(CITY_INFO, stringify(storeData));
                callback(null);
                _removeScript();
            };

            abortTimeout = win.setTimeout(function () {
                callback('timeout');
            }, 1000);
    ...
    function _initDomLoad() {
        if (!_initDomLoadTime) {
            ...
            if (doc.querySelectorAll) {
                var _resources = doc.querySelectorAll('head>link,head>script') || [];
                ...
                each(_resources, function (resource) {
                    ...
                    else if (resource.tagName == 'SCRIPT' && !resource.defer && !resource.async) {
                        url = resource.src;
                    }
    ...
    function _formatXHRData() {
        ...
        for (var i = 0, len = _xhrList.length; i < len; i++) {
            ...
            // 慢ajax
            if (temp.diff > defaultConfig.slowAjaxT && getRandom() <= defaultConfig.slowAjaxP) {
    ...
    function _performanceSend() {
        ...
        try {
            ...
            if (!isObjectEmpty(performance)) {
                ...
                var resourcePerformance = formatResourcePerformanceInfo(pagePerformance.loadEventStartTime > 0 ? pagePerformance.loadEventStartTime : _pageLoadTime - _startTime);
                ...
                function _doSend() {
                    ...
                    if (sourceData.resources.length > 0) {
                        window.setTimeout(function () {
                            post(makeUrl(_defaultHost, _resourcePerformanceUrl), stringify(sourceData));
                        }, 100);
                    }
        ...
        if ((hasDomInited() || needImmediately)) {
            ...
            if (!needImmediately) {
                ...
                if (!_lastSendTime) {
                    ...
                    needImmediately = (now() - _lastSendTime > 10000 || _xhrList.length >= 10);
                }
    ...
    // 业务异常分析
    function _appErrorSend(options) {
        options = options || {};
        var snmb = getCookie('_snmb') || '';
        snmb = decode(snmb) || '';
        var snmbArray = ('' + snmb).split('|') || [];

        //
        var defaultOptions = {
            ct: now(),
            pvid: getCookie('_snmp') || '',
            sid: snmbArray[0] || '',
            ua: encode(_ua),
            url: encode(doc.location.href),
            v: defaultConfig.version,
            vid: snmbArray[1] || '',
            bizid: options['bid'] || '',
            errcode: options['error_code'] || '',
            errdetail: encode(options['error_detail'] || ''), // 需要编码下
            errtype: options['error_type'] || '',
            mbrid: options['member_id'] || '',
            mbrlevel: options['member_level'] || '',
            region: options['region'] || '',
            typname: options['type_name'] || ''
        };

        //
        post(makeUrl(_defaultHost, _appErrorUrl), stringify(defaultOptions))
    }

    // 获取页面基本信息
    function _getPageInfo() {
        var data = getStorage(SELF_DATA) || '';
        try {
            if (data) {
                data = parse(data);
            }
    ...
    function _initBrowserInfo() {
        // 数据存储在 storage 里面
        var storage = getStorage(SELF_DATA) || '';
        ...
        storage['pageRef'] = doc.referrer || '';
        storage['browserLanguage'] = navigator.language || navigator.browserLanguage;
        storage['screenHeight'] = win.screen && win.screen.height;
        storage['screenWidth'] = win.screen && win.screen.width;

        setStorage(SELF_DATA, stringify(storage), timeout);
    }
            ...
            wrap(false, XHR.prototype, 'send', function (event) {
                return function () {
                            ...
                            if (_openWrap) {
                                if (_this.readyState === 4) {
                                    ...
                                    if (_this.responseType == '' || _this.responseType == 'text') {
                                        _openWrap.responseLength = getResourceLength(_this.responseText);
                                    }
                                }
                                ...
                                var xhrItem = {
                                    ...
                                    timeDiff: _openWrap.status > 0 ? _openWrap.end - _openWrap.start : 0,
                                    status: _openWrap.status,
                                    responseLength: _openWrap.responseLength,
                                    requestLength: _openWrap.requestLength,
                                    errorReason: _openWrap.status == 0 ? (cancelReason || 'error') : '',
                    ...
                    if (!this._wrap) {
                        ...
                        this._openWrap.requestLength = arguments[0] ? getResourceLength(arguments[0]) : 0;

                        var result = wrap(false, this, 'onreadystatechange', _wrapReadyStateChange);
    ...
    function _addEvents() {
        ...
        addEventListener(win, 'load', function () {
            ...
            if (_fuckIe) {
                win.setTimeout(function () {
                    if (win.onerror && !win.onerror._sn) {
                        _listenIEError();
                    }
                }, 0);
            }
        ...
        if (_fuckIe) {
            ...
            if (win.addEventListener) {
                addEventListener(win, 'error', _handleWindowError);
            }
        }
        //
        addEventListener(doc, 'scroll', function () {
            setFirstTouchTime();
        });
        //
        addEventListener(doc, 'keypress', function () {
            setFirstTouchTime();
        });
        //
        addEventListener(doc, 'click', function () {
            setFirstTouchTime();
        });

        //
        addEventListener(doc, 'DOMContentLoaded', function () {
            _initDomLoad();
        });
        //
        addEventListener(doc, 'readystatechange', function () {
        ...
        if (performance) {
            ...
            //
            addEventListener(performance, 'webkitresourcetimingbufferfull', function () {
                if (performance.getEntriesByType) {
                    var resource = performance.getEntriesByType('resource');
                    if (resource) {
                        _performanceResourceTimingBufferList = _performanceResourceTimingBufferList.concat(resource);
    ...
    // 
    var config = {
        key: "'##key##'", // 对应服务器端 应用的id
        version: "'##version##'",   // 前端js版本号

        slowPageT: parseInt("'##slowPageT##'", 10) || 7000,
        slowResourceT: parseInt("'##slowResourceT##'", 10) || 4000,
        slowAjaxT: parseInt("'##slowAjaxT##'", 10) || 5000,

        normalP: parseInt("'##normalP##'", 10) || 50,
        slowP: parseInt("'##slowP##'", 10) || 100,
        slowResourceP: parseInt("'##slowResourceP##'", 10) || 100,
        ajaxP: parseInt("'##ajaxP##'", 10) || 50,
        slowAjaxP: parseInt("'##slowAjaxP##'", 10) || 100,
        jsErrorP: parseInt("'##jsErrorP##'", 10) || 100,
        ajaxErrorP: parseInt("'##ajaxErrorP##'", 10) || 100
    };
    ...
    win._snBusError = win._snBusError || {};

    win._snBusError.jsError = _appErrorSend
